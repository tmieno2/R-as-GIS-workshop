...
ggplot() +
  geom_sf(data = ia_nitrogen, aes(fill = acres))
```
...
ggplot() +
  ...
  geom_sf(data = dplyr::filter(ia_nitrogen, county_code %in% huc_07060004$county_code))
```
...
::: {.column width="70%"}
+ Let's now get back to the example of HUC units and county-level nitrogen use data.

+ We would like to find area-weighted average of nitrogen use instead of the simple average.
